...
from models_eco import db, Planning, PlanningPoint, User

planning_bp = Blueprint('eco_planning', __name__, url_prefix='/api/planning')
...
def create_planning():
    ...
    import uuid
    pl = Planning(id=data.get('id') or str(uuid.uuid4())[:8], date_planning=date_planning, shift=data.get('shift', 'matin'), chauffeur_id=data.get('chauffeur_id'), camion_id=data.get('camion_id'), depot_id=data.get('depot_id', 'd1'), dechetterie_id=data.get('dechetterie_id'), trajet_calcule=data.get('trajet_calcule'), status=data.get('status', 'planifie'))
    db.session.add(pl)
    for i, point_id in enumerate(data.get('points_assignes', []) or []):
        db.session.add(PlanningPoint(planning_id=pl.id, point_id=point_id, ordre=i))
    db.session.commit()
...
def marquer_collecte(planning_id):
    ...
    if not pp:
        return jsonify({'error': 'Point non assigné'}), 400
