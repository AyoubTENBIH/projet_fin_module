...
from models_eco import db, Camion, User

camions_bp = Blueprint('eco_camions', __name__, url_prefix='/api/camions')
...
def create_camion():
    ...
    import uuid
    c = Camion(id=str(uuid.uuid4())[:8], immatriculation=immat, capacite=int(data.get('capacite', 5000)), type=data.get('type', 'benne'), annee=data.get('annee'), etat=data.get('etat', 'operationnel'), chauffeur_id=data.get('chauffeur_id'), kilometrage=int(data.get('kilometrage', 0)), derniere_maintenance=data.get('derniere_maintenance'))
    db.session.add(c)
    db.session.commit()
