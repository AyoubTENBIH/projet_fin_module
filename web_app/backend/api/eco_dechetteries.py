...
import json
from models_eco import db, Dechetterie, User

dechetteries_bp = Blueprint('eco_dechetteries', __name__, url_prefix='/api/dechetteries')
...
def create_dechetterie():
    ...
    d = Dechetterie(id=data.get('id') or str(uuid.uuid4())[:8], nom=nom, lat=float(data.get('lat', 33.58)), lng=float(data.get('lng', -7.49)), capacite_max=data.get('capacite_max'), types_acceptes=types_acceptes)
    db.session.add(d)
    db.session.commit()
