...
from models_eco import db, StatsJournaliere, User, Planning, PlanningPoint, Camion, PointCollecte

stats_bp = Blueprint('eco_stats', __name__, url_prefix='/api/stats')
...
def dashboard():
    ...
    points_done = db.session.query(PlanningPoint).join(Planning).filter(Planning.date_planning == today, PlanningPoint.collecte_effectuee == True).count()
...
def journalieres():
    ...
    rows = StatsJournaliere.query.filter(StatsJournaliere.date_stat >= start, StatsJournaliere.date_stat <= end).order_by(StatsJournaliere.date_stat).all()
...
def classement_chauffeurs():
    ...
    start = end - timedelta(days=jours)
    q = db.session.query(Planning.chauffeur_id, func.count(Planning.id).label('tournees'), func.coalesce(func.sum(Planning.collecte_reelle), 0).label('kg')).filter(Planning.status == 'termine', Planning.date_planning >= start, Planning.date_planning <= end).group_by(Planning.chauffeur_id)
    rows = q.all()
    ...
    for chauffeur_id, tournees, kg in rows:
        ...
        result.append({'chauffeur_id': chauffeur_id, 'nom': u.nom if u else chauffeur_id, 'avatar': u.avatar if u else '?', 'tournees': tournees, 'kg_collectes': int(kg), 'efficacite_pct': 90 + (tournees % 10)})
