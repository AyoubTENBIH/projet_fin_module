# -*- coding: utf-8 -*-
"""Contrôleur Points de collecte - CRUD."""
from flask import Blueprint, request, jsonify
from backend.models import db, PointCollecte, User
...
def create_point():
    ...
    import uuid
    p = PointCollecte(
        id=data.get('id') or str(uuid.uuid4())[:8], nom=nom, type=data.get('type', 'collecte'),
        lat=float(data.get('lat', 33.57)), lng=float(data.get('lng', -7.59)),
        volume_moyen=int(data.get('volume_moyen', 0)), frequence=data.get('frequence', 'quotidien'),
        priorite=data.get('priorite', 'normale'), zone_id=data.get('zone_id'),
        horaire_collecte=data.get('horaire_collecte'), contact=data.get('contact'), adresse=data.get('adresse'),
    )
    db.session.add(p)
    db.session.commit()
